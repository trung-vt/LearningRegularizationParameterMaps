...
class StaticImageDenoisingDataset(Dataset):
	...
	def __init__(
		...
		for k, img_id in enumerate(ids):
			...
			# TODO: Choose sample path
			# sample_path = os.path.join(data_path, f"MOT17-{img_id}")
			sample_path = os.path.join(data_path, f"Static-Images-{img_id}")
			if extract_data:
				xf = self.create_static_img(sample_path).unsqueeze(0) / 255
			else:
	...
	def create_static_img(self, sample_path: str):
