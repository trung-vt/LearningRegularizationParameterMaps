...
\begin{algorithm}
\caption{PDHG algorithm for image denoising with fixed regularization parameter-map $\boldsymbol{\Lambda}_\Theta$ (adapted from \cite{81})}
\begin{algorithmic}[1]
...
\begin{figure}[ht]
  ...
  \caption{Chest X-ray example: clean, noisy with $\sigma = 0.5$, $\lambda = 0.07$ with PSNR = 29.945, $\mathbf{\Lambda}_\theta$ with PSNR = \textbf{31.248}}
  \label{fig:enter-label}
\end{figure}
