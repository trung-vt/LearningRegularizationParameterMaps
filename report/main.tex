...
% could  may lead will penalise areas with high variation, typically areas with more details, while lower $\lambda$ values prioritise detail preservation over noise reduction.
...
\begin{algorithm}[H]
\caption{PDHG algorithm for image denoising with fixed regularisation parameter-map $\boldsymbol{\Lambda}$ (adapted from \cite{kofler2023learning})}
\begin{algorithmic}[1]
...
The search can be done more efficiently by employing neural network models that are designed for image processing. In this dissertation we will use the U-Net model, which is a specific design of a convolutional neural network.
