...
where $\mathbf{x}_{\text{true}} \in V^n$ with $V \in \mathbb{R}$ is the object to be imaged, $\mathbf{e} \in V^n$ denotes sopme random noise component, and $\mathbf{z} \in V^n$ represents the measured data.
...
where $\nabla \mathbf{x}$ is the gradient of the image $\mathbf{x}$ and $\lambda$ is a regularisation parameter that controls the trade-off between the total variation of the image and the mean squared error between the denoised image $\mathbf{x}$ and the noisy image $\mathbf{z}$.
