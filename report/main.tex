...
\begin{algorithmic}[1]
\STATE \textbf{Input:} $L = \| [\mathbf{I}, \nabla]^\text{T} \|$, $\tau \lesssim 1/L$, $\sigma \lesssim 1/L$, $\theta \lesssim 1$, noisy image $\mathbf{x}_0$
...
In the Chambolle and Pock (\url{DOI 10.1007/s10851-010-0251-1}) paper, it is proved that if $L^2 \sigma \tau < 1$ then the PDHG algorithm converges to the solution of the primal dual problem.
However, in \url{https://arxiv.org/abs/1111.5632} it was shown that in practice, the algorithm always converges even if $L^2 \sigma \tau \eq 1$.
Since $\sigma$ and $\tau$ are step sizes, choosing them to be large can speed up the convergence of the algorithm.
Still, to be safe, in our implementation we multiply with sigmoid(10) which is about 0.9999546 so that the condition is always satisfied.
