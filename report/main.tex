...
where $\mathbf{x}_{\text{true}} \in \mathbb{R}^n$ is 
the object to be imaged, 
% the "true"/"clean" image,
% denoised image
$\mathbf{e} \in \mathbb{R}^n$ denotes sopme random noise component, and $\mathbf{z} \in \mathbb{R}^n$ represents 
the measured data.
% the noisy image.
...
In particular, in the discrete case, given a matrix $\mathbf{x}$, $\nabla \mathbf{x}$ is two matrices, one for the horizontal gradient and one for the vertical gradient. 

\begin{equation}
  \nabla \mathbf{x} = \begin{bmatrix}
    \nabla_{\text{h}} \mathbf{x} \\
    \nabla_{\text{v}} \mathbf{x}
  \end{bmatrix}
\end{equation}

where $\nabla_{\text{h}} \mathbf{x}$ is the horizontal gradient of the image $\mathbf{x}$ and $\nabla_{\text{v}} \mathbf{x}$ is the vertical gradient of the image $\mathbf{x}$.

For the forward gradient, the horizontal gradient is calculated as:

\begin{equation}
  \nabla_{\text{h}} \mathbf{x} = \begin{bmatrix}
    \mathbf{x}_{1,2} - \mathbf{x}_{1,1} & \mathbf{x}_{1,3} - \mathbf{x}_{1,2} & \ldots & \mathbf{x}_{1,n} - \mathbf{x}_{1,n-1} \\
    \mathbf{x}_{2,2} - \mathbf{x}_{2,1} & \mathbf{x}_{2,3} - \mathbf{x}_{2,2} & \ldots & \mathbf{x}_{2,n} - \mathbf{x}_{2,n-1} \\
    \vdots & \vdots & \ddots & \vdots \\
    \mathbf{x}_{m,2} - \mathbf{x}_{m,1} & \mathbf{x}_{m,3} - \mathbf{x}_{m,2} & \ldots & \mathbf{x}_{m,n} - \mathbf{x}_{m,n-1} \\
  \end{bmatrix}
\end{equation}

and the vertical gradient is calculated as:

\begin{equation}
  \nabla_{\text{v}} \mathbf{x} = \begin{bmatrix}
    \mathbf{x}_{2,1} - \mathbf{x}_{1,1} & \mathbf{x}_{3,1} - \mathbf{x}_{2,1} & \ldots & \mathbf{x}_{m,1} - \mathbf{x}_{m-1,1} \\
    \mathbf{x}_{2,2} - \mathbf{x}_{1,2} & \mathbf{x}_{3,2} - \mathbf{x}_{2,2} & \ldots & \mathbf{x}_{m,2} - \mathbf{x}_{m-1,2} \\
    \vdots & \vdots & \ddots & \vdots \\
    \mathbf{x}_{2,n} - \mathbf{x}_{1,n} & \mathbf{x}_{3,n} - \mathbf{x}_{2,n} & \ldots & \mathbf{x}_{m,n} - \mathbf{x}_{m-1,n} \\
  \end{bmatrix}
\end{equation}
...
The following is the formula for the forward gradient:
...
We can see that the regularisation parameter needs not to be a scalar $\lambda$, but can be a diagonal matrix $\Lambda \in \mathbb{R}^{n}$.
...
if we use a matrix $\Lambda$ of regularisation parameters.

In this thesis, we will use the later form of the regularisation term.
The U-Net will be used to find the regularisation parameter $\Lambda$ that produces the best denoised image for any particular noisy image.
