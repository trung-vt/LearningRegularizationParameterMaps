...
\begin{equation}
  \mathcal{R}(\mathbf{x}) = \| \Lambda \nabla \mathbf{x} \|_1
  = \| \Lambda^{(x)} \nabla_x \mathbf{x} \|_1 + \| \Lambda^{(y)} \nabla_y \mathbf{x} \|_1
\end{equation}

where $\Lambda^{(x)}$ and $\Lambda^{(y)}$ are matrices that contain the regularisation parameters for the vertical and horizontal gradients respectively.
