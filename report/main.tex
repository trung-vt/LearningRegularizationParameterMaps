...
where $\mathbf{x}_{\text{true}} \in V^n$ with $V \in \mathbb{R}$ is 
the object to be imaged, 
% the "true"/"clean" image,
% denoised image
$\mathbf{e} \in V^n$ denotes sopme random noise component, and $\mathbf{z} \in V^n$ represents 
the measured data.
% the noisy image.
...
\begin{equation}
  \mathbf{x}_{\text{denoised}} = \arg \min_{\mathbf{x}} d(\mathbf{x}, \mathbf{z}) + \mathit{R}(\mathbf{x})
\end{equation}

where $d(\cdot, \cdot)$ is a data fidelity term and $\mathit{R}(\cdot)$ is a regularisation term.
...
The regularisation term $\mathit{R}(\cdot)$ is the total variation of the image, which is a measure of the amount of variation in the intensity of the image.
The total variation of an image can be calculated using the following formula:

\begin{equation}
  \mathit{R}(\mathbf{x}) = \lambda \| \nabla \mathbf{x} \|_1
\end{equation}

where $\nabla \mathbf{x}$ is the gradient of the image $\mathbf{x}$ and $\lambda > 0$ is a regularisation parameter that controls the trade-off between the total variation of the image and the mean squared error between the denoised image $\mathbf{x}$ and the noisy image $\mathbf{z}$.


This is the discrete case, and $\nabla \mathbf{x}$ can be the calculated in four different ways: forward, backward, central, and Sobel. 
We can treat $\mathbf{x}$ as a matrix and calculate the gradient in the horizontal and vertical directions separately, and then combine them to get the total variation of the image.
Let $I$ represent $\mathbf{x}$, then:

\begin{equation}
  \| \nabla \mathbf{x} \|_1 = \text{TV}(I) = \sum_{i,j} |I_{i,j} - I_{i+1,j}| + |I_{i,j} - I_{i,j+1}|
\end{equation}

where $I_{i,j}$ is the intensity value of pixel $(i,j)$ in the image $I$. (Note: $I$ looks like the identity matrix. Is there a better notation?)
...
We can see that the regularisation parameter needs not to be a scalar $\lambda$, but can be a matrix $\Lambda$.
