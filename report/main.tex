...
% The total variation (TV) method is a method for image denoising that is based on the principle of minimising the total variation of the image.
% The total variation of an image is a measure of the amount of variation in the intensity of the image.
% The TV approach works by finding the image that has the smallest total variation and is closest to the noisy image in terms of the mean squared error.
...
% recover $\mathbf{x}_{\text{true}}$ or at least a good enough reconstruction given $\mathbf{z}$.
...
%  calculate the gradient in the horizontal and vertical directions separately, and then combine them to get the total variation of the image. 
...
% Note that $\mathbf{x}_{i,j}$ can be a scalar for a black-and-white image or a vector of 3 values for a colour image.
...
% could  may lead will penalize areas with high variation, typically areas with more details, while lower $\lambda$ values prioritize detail preservation over noise reduction.
...
\begin{algorithm}[H]
\caption{PDHG algorithm for image denoising with fixed regularization parameter-map $\boldsymbol{\Lambda}$ (adapted from \cite{kofler2023learning})}
\begin{algorithmic}[1]
...
Note that this algorithm can also be applied to the single scalar $\lambda$ case by simply setting all values of $\mathbf{\Lambda}$ to $\lambda$.

The choice of $L$ is taken from \cite{Sidky_2012}.
...
more efficiently.
% However, this approach is not perfect, as it might leave noise in detailed areas.
The search can be done more efficiently by employing neural network models that are designed for image processing. In this disseration we will use the U-Net model, which is a specific design of a convolutional neural network.
...
% We will compare the results to the best-case scenario if we were to use a scalar regularisation parameter $\lambda$.

% The performance of Primal-Dual Hybrid Gradient (PDHG) methods can be improved by assigning suitable regularisation parameters to regions based on their characteristics. However, 

% To address this, we can apply image segmentation. 
% Effective image segmentation helps determine appropriate $\lambda$ values for different regions. 
% A good network for learning image segmentation is U-Net \cite{ronneberger2015unet}.
...
% This structure is specifically designed for image segmentation, where the network needs to not only extract features but also localize them precisely within the image.
...
% For our implementation, we consider $256 \times 256$.
...
We used the Chest X-Ray Images (Pneumonia) dataset which was downloaded from \cite{kermany2018large}.
...
The clean and orignal versions as well as the resulting denoised images are shown in figure 
% \ref{fig:compare}.
4.
...
This would help to reduce the stair-case effect in the denoised image, which we can see more prevalent in the denoised image using the scalar $\lambda$.
...
Figure 8 shows the histograms of the differences in PSNR and SSIM values between the two methods.
We can see that the difference is positive in all cases, which shows that the regularisation parameter map consistently produces higher PSNR and SSIM values than the scalar $\lambda$.
