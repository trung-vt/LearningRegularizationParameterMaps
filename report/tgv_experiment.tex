...
% $\text{clip}_{\boldsymbol{\Lambda}}$ 
% is a function that clips the values of $\bm{y}$ to the range $[\mathbf{0}, \boldsymbol{\Lambda}]$.
...
% We used the SeaTurtleID2022 data set \cite{Adam_2024_WACV}.
...
\codeword{random_noise} function from the well-established \codeword{scikit-image} library, version \codeword{0.24.0}.
...
\begin{figure}[H]
    ...
    % Example of the transformations and noisy versions of one color image.
...
The TGV method is better than the TV method using a scalar $\lambda$.
It is even better then the TV method using the regularisation parameter map $\mathbf{\Lambda}_{\Theta}$.

We improved it even further using U-Net to learn two regularisation parameter maps $\mathbf{\Lambda}_1$ and $\mathbf{\Lambda}_0$.
